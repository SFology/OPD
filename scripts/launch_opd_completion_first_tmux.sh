...
STABLE_SAMPLES=3
POLL_SECONDS=30
SESSION=opd-original-completion-first
WORKER=false
ORCH_LOG=
...
select_stable_gpus() {
    local stable=0 previous= current= records=
    while true; do
        records=$(nvidia-smi \
...
if [[ "$MODE" == probe || "$MODE" == probe-then-full ]]; then
    echo "Running one-step full-shape probe"
    run_managed "$PROBE_CONFIG"
    echo "Probe completed successfully at $(date -u --iso-8601=seconds)"
fi

if [[ "$MODE" == full || "$MODE" == probe-then-full ]]; then
    echo "Running full one-epoch OPD reproduction"
    run_managed "$FULL_CONFIG"
    echo "Full reproduction completed successfully at $(date -u --iso-8601=seconds)"
fi
