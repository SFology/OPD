...
CONFIG=$REPO/configs/trustworthy_opd/objective_reliability_calibration.yaml
SESSION=opd-objective-reliability
RUN_DIR=
WORKER=false
ORCH_LOG=
...
if [[ "$WORKER" != true ]]; then
    ...
    "$TMUX_BIN" new-session -d -s "$SESSION" \
        "bash -lc '$worker_command; rc=\$?; echo objective_reliability_exit=\$rc; exec bash'"
...
[[ -n "$ORCH_LOG" ]] || { echo "--orchestration-log is required" >&2; exit 2; }
...
if [[ -n "$(git status --porcelain)" ]]; then
    echo "Repository must be clean before the objective reliability run" >&2
    git status --short >&2
    exit 1
fi
...
echo "Starting objective reliability study at $(date -u --iso-8601=seconds)"
echo "Frozen revision: $FROZEN_HEAD"
command=(python -u scripts/trustworthy_opd/run_objective_reliability_calibration.py)
if [[ -n "$RUN_DIR" ]]; then
