...
CONFIG="$REPO_ROOT/configs/trustworthy_opd/ppl_decode_control.yaml"
RUN_DIR=""
SESSION="opd-ppl-decode-control-$(date -u +%Y%m%d-%H%M%S)"
ATTACH=false
...
mkdir -p "$RUN_DIR/ppl_decode_control/logs"
STAMP="$(date -u +%Y%m%d_%H%M%S)"
LOG="$RUN_DIR/ppl_decode_control/logs/launch_${STAMP}.log"
...
python -u scripts/trustworthy_opd/run_ppl_decode_control.py \
--run-dir $RUN_Q --config $CONFIG_Q \
2>&1 | tee $LOG_Q; \
rc=\${PIPESTATUS[0]}; echo ppl_decode_control_exit=\$rc; exec bash"
