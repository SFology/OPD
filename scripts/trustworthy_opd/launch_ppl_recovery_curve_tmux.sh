...
CONFIG="$REPO_ROOT/configs/trustworthy_opd/ppl_recovery_curve.yaml"
RUN_DIR=""
SESSION="opd-ppl-recovery-$(date -u +%Y%m%d-%H%M%S)"
ATTACH=false
...
mkdir -p "$RUN_DIR/ppl_recovery_curve/logs"
STAMP="$(date -u +%Y%m%d_%H%M%S)"
LOG="$RUN_DIR/ppl_recovery_curve/logs/launch_${STAMP}.log"
...
python -u scripts/trustworthy_opd/run_ppl_recovery_curve.py \
--run-dir $RUN_Q --config $CONFIG_Q \
2>&1 | tee $LOG_Q; \
rc=\${PIPESTATUS[0]}; echo ppl_recovery_exit=\$rc; exec bash"
